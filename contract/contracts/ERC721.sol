...
abstract contract ERC721 is IERC721, IERC165 {
    ...
    modifier tokenIsFrom(uint256 _tokenId, address _from) {
        if (_tokenOwners[_tokenId] != _from)
            revert TokenOwnerMissMatch(_tokenId, _from);
        _;
    }
    ...
    function _mint(
        address _to,
        uint256 _tokenId
    ) internal virtual addressIsNotZero(_to) {
    ...
    function balanceOf(
        address _owner
    ) public view addressIsNotZero(_owner) returns (uint256) {
    ...
    function ownerOf(
        uint256 _tokenId
    ) external view tokenExists(_tokenId) returns (address) {
    ...
    function setApprovalForAll(
        address _operator,
        bool _approved
    ) external addressIsNotZero(_operator) {
